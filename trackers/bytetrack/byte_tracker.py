...
class BYTETracker(object):
    def __init__(self, track_thresh, track_buffer, match_thresh, mot20=True, frame_rate=30):
    ...
    def update(self, dets, _):
        ...
        # offline_stracks = [track for track in self.tracked_stracks if not track.is_activated]
        # online_lost_stracks = [track for track in self.lost_stracks if track.is_activated]
        # offline_lost_stracks = [track for track in self.lost_stracks if not track.is_activated]
        # if is_unconfirmed:
        #     for track in self.lost_stracks:
        #         time_since_update = self.frame_id - track.end_frame
        #         if time_since_update < cfg.tracker.time_since_update_threshold:
