...
class BoostTrack(object):
    ...
    def update(self, dets, _):
        ...
        clss = np.zeros((len(self.trackers), 1))

        for t in range(len(trks)):
            pos = self.trackers[t].predict()[0]
            confs[t] = self.trackers[t].get_confidence()
            clss = self.trackers[t].cls
            trks[t] = [pos[0], pos[1], pos[2], pos[3], confs[t, 0], clss]
        ...
        for trk in reversed(self.trackers):
            ...
            if (trk.time_since_update < 1) and (trk.hit_streak >= self.min_hits or self.frame_count <= self.min_hits):
                ...
                ret.append(np.concatenate((d, [trk.id + 1], [trk.cls], [trk.get_confidence()])).reshape(1, -1))
            i -= 1
